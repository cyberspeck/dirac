...
const allTools = new Set(["edit_file", "write_to_file", "execute_command"])
...
describe("formatResponse.writeToFileMissingContentError", () => {
	describe("first failure (tier 1)", () => {
		it("should include the file path in the error message", () => {
			const result = formatResponse.writeToFileMissingContentError("src/index.ts", 1, allTools)
		...
		it("should include helpful suggestions", () => {
			...
			result.should.containEql("edit_file")
		...
		it("should suggest using edit_file for existing files", () => {
			const result = formatResponse.writeToFileMissingContentError("src/index.ts", 1, allTools)
			result.should.containEql("prefer edit_file to make targeted edits")
		})
		...
		it("should work with different file paths", () => {
			const result = formatResponse.writeToFileMissingContentError("components/MyComponent.tsx", 1, allTools)
	...
	describe("second failure (tier 2)", () => {
		it("should indicate this is the 2nd failed attempt", () => {
			const result = formatResponse.writeToFileMissingContentError("src/index.ts", 2, allTools)
	...
	describe("third+ failure (tier 3)", () => {
		it("should include CRITICAL language on third failure", () => {
			const result = formatResponse.writeToFileMissingContentError("src/index.ts", 3, allTools)
		...
		it("should show correct count for higher failure counts", () => {
			const result = formatResponse.writeToFileMissingContentError("src/index.ts", 5, allTools)
	...
	describe("context window awareness", () => {
		it("should include context warning when usage exceeds 50%", () => {
			const result = formatResponse.writeToFileMissingContentError("src/index.ts", 1, allTools, 60)
		...
		it("should not include context warning when usage is 50% or below", () => {
			const result = formatResponse.writeToFileMissingContentError("src/index.ts", 1, allTools, 50)
			result.should.not.containEql("% full")
		})

		it("should not include context warning when percent is undefined", () => {
			const result = formatResponse.writeToFileMissingContentError("src/index.ts", 1, allTools, undefined)
			result.should.not.containEql("% full")
		})

		it("should show high context window usage correctly", () => {
			const result = formatResponse.writeToFileMissingContentError("src/index.ts", 1, allTools, 85)
			result.should.containEql("85% full")
		})

		it("should include context warning in tier 3 messages", () => {
			const result = formatResponse.writeToFileMissingContentError("src/index.ts", 3, allTools, 75)
