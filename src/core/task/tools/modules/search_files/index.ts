...
export const search_files_spec: DiracToolSpec = {
	...
	name: "search_files",
	description:
		"Regex search across files in the specified paths (files or directories). Skips non-useful content (.git, node_modules, build artifacts, etc. and all files and directories starting with a dot). Prefer `inspect_ast` over this when reasonable. Prefer `edit_ast` over this when reasonable.",
	parameters: [
		{
			name: "paths",
			required: true,
...
export class SearchFilesTool implements IDiracTool<SearchFilesArgs, string> {
	...
	private formatSearchFailureMessage(failedResults: SearchExecutionResult[]): string {
		const details = failedResults
			.map((result) => {
				const label = result.workspaceName ? `${result.workspaceName} (${result.absolutePath})` : result.absolutePath
