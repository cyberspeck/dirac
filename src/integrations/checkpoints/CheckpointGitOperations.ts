...
export class GitOperations {
	private cwd: string
	private nestedGitCache: string[] | undefined
	...
	public async renameNestedGitRepos(disable: boolean) {
		...
		if (disable || !this.nestedGitCache) {
			// Find all .git directories that are not at the root level
			gitPaths = await globby("**/.git" + (disable ? "" : GIT_DISABLED_SUFFIX), {
				cwd: this.cwd,
			...
			if (disable) {
				this.nestedGitCache = gitPaths
			}
		} else {
			// Use cached paths for re-enabling to avoid another expensive globby scan
			gitPaths = this.nestedGitCache.map((p) => p + GIT_DISABLED_SUFFIX)
		}

		// For each nested .git directory, rename it based on operation in parallel
		await Promise.all(
		...
		if (!disable) {
			this.nestedGitCache = undefined // Clear cache after re-enabling
		}
	}
	...
	public async addCheckpointFiles(git: SimpleGit): Promise<CheckpointAddResult> {
		...
		try {
			...
			// Optimization: Use git status --porcelain to find changed files
			// This is much faster than git add . in large repositories
			const status = await git.status(["--porcelain", "--untracked-files=all"])
			const filesToStage = status.files.map((f) => f.path)

			if (filesToStage.length === 0) {
				Logger.debug("No files to stage for checkpoint")
				return { success: true }
			}

			// Attempt to add only changed files. Any files with permissions errors will not be added,
			// but the process will proceed and add the rest (--ignore-errors).
			try {
				// If there are many files, git add . might still be faster or safer due to command line length limits
				if (filesToStage.length > 100) {
					await git.add([".", "--ignore-errors"])
				} else {
					await git.add([...filesToStage, "--ignore-errors"])
				}

				const durationMs = Math.round(performance.now() - startTime)
				Logger.debug(`Checkpoint add operation completed in ${durationMs}ms for ${filesToStage.length} files`)
				return { success: true }
			} catch (_error) {
				// Fallback to git add . if targeted add fails
				try {
					await git.add([".", "--ignore-errors"])
					return { success: true }
				} catch (fallbackError) {
